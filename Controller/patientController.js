...
exports.addPatient = async (request, response, next) => {
  try {
    ...
    await patient.save();
    response.status(201).json({ message: "Patient created successfully.", patient });
...
exports.editPatient = async (request, response, next) => {
  try {
    const patient = await PatientSchema.findByIdAndUpdate(request.params.id, request.body, { new: true });
    response.status(200).json({ message: "Patient updated successfully.", patient });
  } catch (error) { next(error); }
};
...
exports.removePatient = async (request, response, next) => {
  try {
    const patient = await PatientSchema.findByIdAndDelete(request.params.id || request.body.id);
    if (!patient) { return next(new Error('patient not found')); }
    response.status(201).json({ message: "Patient removed successfully.", patient });
  } catch (error) { next(error); }
};
