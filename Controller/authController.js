    ...
    try {
        ...
        if (isAdmin) {
            ...
            if (isPatient) {
                ...
                const isEmployee = await Employee.findOne({ name: username });
                if (isEmployee) {
                    const isMatch = await bcrypt.compare(password, isEmployee.password);
                    if (isMatch) {
                        token = jwt.sign({ role: "employee", username: username, id: isEmployee._id }, process.env.SECRET_KEY, { expiresIn: "1h" });
                        response.status(200).json({ message: "Employee login successful", token });
                    } else { return next(new Error('Invalid Credentials')); }
                } else { return next(new Error('Invalid Credentials')); }
            }
        }
